...
def GetFeaturesSALIB_THX(rates,params,mode="rates"):
    ...
    if 'ctc0_month' in params:
        ctc0_month = change(close_month[0], close_month[1], "direction")  # direction of CTC (above or below 0)
        df["ctc0_month"] = ctc0_month
    if 'ctc1_month' in params:
        ctc1_month = change(close_month[1], close_month[2],"direction")  # direction of CTC (above or below 0)
        df["ctc1_month"] = ctc1_month
    if 'ctc2_month' in params:
        ctc2_month = change(close_month[2], close_month[3],"direction")  # direction of CTC (above or below 0)
    ...
    if 'range0_h1' in params:
        range0_h1 = change(high_h1[0], low_h1[0], "percent")  # daily range
        df["range0_h1"] = range0_h1
    if 'range1_h1' in params:
        range1_h1 = change(cmax(high_h1[0:2]), cmin(low_h1[0:2]), "percent")  # daily range
        df["range1_h1"] = range1_h1
    if 'range2_h1' in params:
        range2_h1 = change(cmax(high_h1[0:3]), cmin(low_h1[0:3]), "percent")  # daily range
        df["range2_h1"] = range2_h1
    if 'range4_h1' in params:
        range4_h1 = change(cmax(high_day[0:5]), cmin(low_day[0:5]), "percent")  # daily range
    ...
    if 'range0c' in params:
        range0c = change(high_m15[0], low_m15[0], "percent")     # daily range
        df["range0c"] = range0c
    if 'range1c' in params:
        range1c = change(high_m15[1], low_m15[1], "percent")  # daily range
        df["range1c"] = range1c
    if 'range2c' in params:
        range2c = change(cmax(high_m15[0:3]), cmin(low_m15[0:3]), "percent")  # daily range
        df["range2c"] = range2c
    if 'range4c' in params:
        range4c = change(cmax(high_m15[0:5]), cmin(low_m15[0:5]), "percent")  # daily range
        df["range4c"] = range4c
    if 'range8c' in params:
        range8c = change(cmax(high_m15[0:9]), cmin(low_m15[0:9]), "percent")  # daily range
        df["range8c"] = range8c
    if 'range16c' in params:
        range16c = change(cmax(high_m15[0:17]), cmin(low_m15[0:17]), "percent")  # daily range
    ...
    if 'ibs1_label2_e' in params:
        ibs1_label2_e = label(price_to_range(close, cmax(high_day[0:2]), cmin(low_day[0:2])),[0.3,0.7])  
    ...
    if 'volume2_5_10' in params:
        volume_ma_fast = talib.MA(volume, timeperiod=5, matype=0)
        volume_ma_slow = talib.MA(volume, timeperiod=10, matype=0)
        volume_ma_fast = np.array([(volume_ma_fast[i] if volume_ma_fast[i]>0 else 1) for i in range(0,len(volume_ma_fast))]) 
        volume_ma_slow = np.array([(volume_ma_slow[i] if volume_ma_slow[i] > 0 else 1) for i in range(0, len(volume_ma_slow))])  
    ...
    if 'pattern' in params:
        ...
        df["pattern"] = pattern
        pattern2 = talib.CDLDOJISTAR(open, high, low, close) 
        df["pattern2"] = pattern2
        pattern3 = talib.CDLMORNINGSTAR(open, high, low, close, penetration=0)  
        df["pattern3"] = pattern3
        pattern4 = talib.CDLUNIQUE3RIVER(open, high, low, close)  
        df["pattern4"] = pattern4
        pattern5 = talib.CDL3WHITESOLDIERS(open, high, low, close)  
